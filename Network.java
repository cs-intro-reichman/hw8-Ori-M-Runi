...
public class Network {
    ...
    public User getUser(String name) {
        for (int i = 0; i < userCount; i++) {
            // Since users cannot be removed from the network, there will be no user beyond the null value
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        String nameToFollow = null;
        int maxMutual = 0;
        for (int i = 0; i < userCount; i++) {
            if (users[i].getName().equals(name)) {continue;}
            int mutualCount = user1.countMutual(users[i]);
    ...
    public String mostPopularUser() {
        ...
        for (int i = 0; i < userCount; i++) {
            int followsCount = followeeCount(users[i].getName());
            if (followsCount > mostFollows) {
                mostFollows = followsCount;
